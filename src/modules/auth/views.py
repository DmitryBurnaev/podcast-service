...
from starlette_web.contrib.auth.views import JWTSessionMixin as StarletteWebJWTSessionMixin
...
class JWTSessionMixin(StarletteWebJWTSessionMixin):
    """Allows to update session and prepare usual / refresh JWT tokens"""

    request: PRequest

    async def _create_session(self, user: User) -> TokenCollection:
        token_collection = await super()._create_session(user)
        await register_ip(self.request)
