...
class DownloadEpisodeTask(RQTask):
    ...
    async def perform_run(self, episode_id: int) -> FinishCode:
        ...
        await self._update_episodes(episode.remote_id, update_data={"status": status.DOWNLOADING})
        ...
        await self._update_all_rss(episode.remote_id)

        podcast_utils.delete_file(settings.TMP_AUDIO_PATH / result_filename)

        logger.info("=== [%s] DOWNLOADING total finished ===", episode.remote_id)
        return FinishCode.OK

    async def _check_is_needed(self, episode: Episode):
        """Allows to find another episodes with same `remote_id` which were already downloaded."""
    ...
    async def _download_episode(self, episode: Episode):
        ...
        try:
            ...
            await Episode.async_update(
                ...
                filter_kwargs={"remote_id": episode.remote_id},
                update_data={"status": Episode.Status.ERROR},
            )
            raise DownloadingInterrupted(code=FinishCode.ERROR)

        logger.info("=== [%s] DOWNLOADING was done ===", episode.remote_id)
    ...
    async def _upload_file(self, episode: Episode, result_filename: str):
        ...
        logger.info("=== [%s] UPLOADING was done === ", episode.remote_id)

    async def _update_all_rss(self, remote_id: str):
        """Allows to regenerate rss for all podcast with requested episode (by remote_id)"""

        logger.info("Episodes with source #%s: updating rss for all podcast", remote_id)
        affected_episodes = await Episode.async_filter(self.db_session, remote_id=remote_id)
    ...
    async def _update_episodes(self, remote_id: str, update_data: dict):
        """Allows to update data for episodes (filtered by remote_id)"""

        filter_kwargs = {"remote_id": remote_id, "status__ne": status.ARCHIVED}
    ...
    async def _upload_cover(self, episode: Episode, tmp_path: Path) -> str:
        ...
        while attempt := (attempt - 1):
            if result_url := self.storage.upload_file(
                ...
                filename=Episode.generate_image_name(episode.remote_id),
            ):
                return result_url
