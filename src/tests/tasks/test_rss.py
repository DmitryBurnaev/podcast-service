...
class TestGenerateRSSTask:
    ...
    def test_generate__single_podcast__ok(self, user, mocked_s3, db_session):
        ...
        podcast_1: Podcast = async_run(Podcast.async_create(**get_podcast_data()))
        podcast_2: Podcast = async_run(Podcast.async_create(**get_podcast_data()))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.NEW
        episode_new = async_run(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.DOWNLOADING
        episode_downloading = async_run(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.PUBLISHED
        episode_data["published_at"] = datetime.now()
        episode_published = async_run(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_2, creator=user)
        episode_data["status"] = Episode.Status.PUBLISHED
        episode_podcast_2 = async_run(Episode.async_create(**episode_data))
