...
class TestGenerateRSSTask:
    ...
    def test_generate__single_podcast__ok(self, user, mocked_s3, db_session):
        ...
        podcast_1: Podcast = await_(Podcast.async_create(**get_podcast_data()))
        podcast_2: Podcast = await_(Podcast.async_create(**get_podcast_data()))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.NEW
        episode_new = await_(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.DOWNLOADING
        episode_downloading = await_(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_1, creator=user)
        episode_data["status"] = Episode.Status.PUBLISHED
        episode_data["published_at"] = datetime.now()
        episode_published = await_(Episode.async_create(**episode_data))

        episode_data = get_episode_data(podcast_2, creator=user)
        episode_data["status"] = Episode.Status.PUBLISHED
        episode_podcast_2 = await_(Episode.async_create(**episode_data))
        ...
        podcast_1 = await_(Podcast.async_get(db_session, id=podcast_1.id))
    ...
    def test_generate__upload_failed__fail(self, podcast, mocked_s3, db_session):
        ...
        podcast_1 = await_(Podcast.async_get(db_session, id=podcast.id))
